...
### Теория: экран — это цепочка нескольких запросов

Пользователь видит одну операцию — например, «сдать тест». Для браузера и
Django это обычно несколько отдельных событий: GET формы, POST ответов, 302
redirect, GET результата. Между этими запросами нет общего экземпляра view;
связь создают URL, session и изменения в БД.

Если учиться смотреть на Network как на сценарий, становятся понятными
формы, flash messages и POST/Redirect/GET. Если смотреть только на один
template, кажется, будто кнопка «магически» обновляет страницу.
...
## 10.13. Как объяснять результат каждого шага

При ручном прогоне не ограничивайтесь статусом 200. Для каждого клика
проверяйте четыре уровня:

| Уровень | Вопрос | Пример для «Записаться на курс» |
|---|---|---|
| Browser | какую кнопку нажали? | submit в public course detail |
| HTTP | какой request отправлен? | POST `/students/enroll-course/` |
| View | какой метод выполнился? | `StudentEnrollCourseView.form_valid()` |
| Data/UI | что поменялось? | M2M enrollment + redirect к materials |

### Пример: почему после кнопки модуль всё ещё на той же странице

После «Отметить модуль пройденным» пользователь может подумать, что view
ничего не сделала, потому что URL после redirect снова похож:

```text
/students/course/1/module/2/
```

Но в Network должны быть **два** запроса:

```text
1. POST /students/course/1/module/2/complete/ → 302
2. GET  /students/course/1/module/2/          → 200
```

Между ними `ModuleCompleteView.post()` записал ModuleProgress, а
`StudentCourseDetailView.get_context_data()` на втором запросе пересчитал
progress. UI-следствия:

- progress bar меняет ширину;
- sidebar может показать `✓`;
- `base.html` отображает success/error flash message.

Этот принцип — «POST меняет данные, следующий GET показывает новое состояние»
— нужно искать на каждом form screen в приложении.
