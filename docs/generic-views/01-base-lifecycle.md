...
### Теория: request — это не состояние view

HTTP-запрос независим от предыдущего. Сервер не «помнит», какую страницу
студент открывал до этого, только потому что он держит класс `DetailView`.
Django создаёт новый экземпляр CBV на каждый request именно для того, чтобы
не смешивать `self.object`, `self.course` и `self.request` между
пользователями.

Состояние между запросами хранится в других местах: в БД, session, cookie,
cache или параметрах URL. Например, после нажатия «Записаться на курс»
`StudentEnrollCourseView` сохраняет M2M-связь в БД, затем redirect открывает
новый request `StudentCourseDetailView`, который читает уже изменённую БД.
Это объясняет архитектуру POST/Redirect/GET и то, почему нельзя рассчитывать
на `self.course` из предыдущего запроса.
...
## 1.11. Где базовый lifecycle уже работает в Educa

Хотя `setup()` и `dispatch()` редко видны в коде, они запускаются **на каждом
экране** проекта. Для любого URL путь одинаков в начале:

```text
URL pattern
→ MyView.as_view()
→ создаётся новый instance MyView
→ setup(request, *args, **kwargs)
→ dispatch(request, *args, **kwargs)
→ get() / post() / другой обработчик
```

### Реальный пример: выбранный модуль студента

На странице материалов студент нажимает название модуля в sidebar:
...
```text
GET /students/course/7/module/3/
```

До выполнения `StudentCourseDetailView.get_context_data()` Django уже сделал:

```python
self.request            # текущий GET HttpRequest
self.args               # ()
self.kwargs             # {"pk": 7, "module_id": 3}
self.object             # Course(pk=7), если DetailView успешно его нашёл
```

А `StudentCourseDetailView.get_context_data()` использует:

```python
module_id = self.kwargs.get("module_id")
context["module"] = self.object.modules.get(pk=module_id)
```

**Что увидит студент:** тот же экран материалов, но выбранным будет модуль 3.
После `return context` `DetailView` рендерит
`students/templates/students/course/detail.html`.

### Когда в Educa нужен свой `dispatch()`

Экран управления модулями преподавателя:

```text
GET или POST /course/7/modules/
```

Должен проверять, что course 7 принадлежит текущему teacher **до** показа
formset и **до** сохранения formset. Поэтому `CourseModuleUpdateView` получает
course в `dispatch()`:
...
При GET это защищает экран редактирования. При POST эта же проверка не даёт
сохранить изменения в чужой курс простым ручным POST-запросом.

### Практическая диагностика lifecycle

1. Откройте DevTools → Network.
2. Нажмите модуль в sidebar.
3. Увидите GET с двумя kwargs в URL.
4. Поставьте breakpoint в `get_context_data()` либо временный `print(self.kwargs)`.
5. В терминале runserver увидите `GET ... 200`.

Если вместо нужного `get()` вызывается другой handler, сначала сопоставьте
HTTP method из Network с методом класса. Если `self.kwargs` пусты — проверьте
имена параметров в `path(...)`.
